...
또한 각 생성 단계에서 발생한 오류와 성공/실패 상태를 \`brand\_result.json\`에 기록하여 일부 단계에서 오류가 발생하더라도 전체 프로세스를 중단하지 않고 다음 단계로 계속 진행할 수 있도록 구성했습니다.
...
\* \`logo\_1.png\`, \`logo\_2.png\`, \`logo\_3.png\`로 저장
...
\| \`load\_env()\`                     | API Key와 Base URL 등 환경변수 확인    |

\| \`get\_user\_input()\`               | 브리프 파일 경로와 출력 폴더 입력            |

\| \`load\_brief()\`                   | 브리프 JSON 로드 및 필수 항목 검증         |

\| \`call\_codyssey\_text()\`           | Codyssey 텍스트 API 호출            |

\| \`parse\_json\_response()\`          | LLM 응답에서 JSON 데이터 추출           |

\| \`generate\_structured\_result()\`   | API 응답 파싱, Pydantic 검증, 재질문 처리 |

\| \`generate\_naming()\`              | 브랜드 네이밍 생성                     |

\| \`generate\_slogans()\`             | 슬로건 생성                         |

\| \`generate\_brand\_story()\`         | 브랜드 스토리 생성                     |

\| \`generate\_competitor\_analysis()\` | 경쟁사 분석 생성                      |

\| \`generate\_color\_palette()\`       | 컬러 팔레트 생성                      |

\| \`palette\_to\_image()\`             | 컬러 HEX 검증 및 이미지 시각화            |

\| \`generate\_logos()\`               | 이미지 API 호출 및 PNG 저장            |

\| \`safe\_generate()\`                | 각 단계의 오류 처리 및 상태 기록            |

\| \`save\_result()\`                  | 최종 결과 JSON 저장                  |
...
└── call\_codyssey\_text()

응답 처리

└── parse\_json\_response()

구조 검증 및 재질문

└── generate\_structured\_result()
...
├── generate\_brand\_story()

├── generate\_competitor\_analysis()

└── generate\_color\_palette()
...
각 생성 단계는 \`safe\_generate()\`를 통해 공통적으로 오류를 처리합니다.
...
텍스트 API와 이미지 API 호출에서 일시적인 네트워크 오류나 API 오류가 발생할 경우 재시도할 수 있도록 처리했다.

재시도 전에는 지수 백오프(Exponential Backoff)를 적용한다.

- 첫 번째 재시도 전: 1초 대기
- 두 번째 재시도 전: 2초 대기
- 세 번째 재시도 전: 4초 대기

현재 프로젝트는 단계별 재시도를 1회로 제한한다.

#### 텍스트 API

LLM 응답의 JSON 파싱 또는 Pydantic 구조 검증에 실패하면 오류 내용을 포함하여 올바른 JSON 형식으로 다시 요청한다. 재요청 전 지수 백오프를 적용한다.

재시도 후에도 실패하면 해당 단계는 `failed` 상태가 되고 오류 내용은 `brand_result.json`의 `errors` 배열에 기록한다.

#### 이미지 API

로고는 각 이미지 생성 요청을 독립적으로 처리한다.

특정 로고 생성 API 호출이 실패하면 지수 백오프 후 1회 재시도한다. 재시도에도 실패하면 해당 로고는 건너뛰고 다음 로고 생성을 계속한다.

이미 생성에 성공한 로고 파일은 그대로 보존하며, 실패한 로고의 오류 정보는 `brand_result.json`의 `errors` 배열에 기록한다.

따라서 일부 API 호출이 실패하더라도 성공한 결과까지 함께 손실되지 않도록 설계했다.
...
유효하지 않은 색상값이 들어오면 오류를 발생시키고 \`safe\_generate()\`를 통해 해당 단계의 실패 상태를 기록합니다.
...
모든 생성 단계가 끝나면 결과를 \`brand\_result.json\`에 저장합니다.
...
\`brand\_result.json\`에는 다음 정보가 포함됩니다.
...
CODYSSEY\_API\_KEY=your\_api\_key

CODYSSEY\_BASE\_URL=your\_base\_url
...
최종적으로 사용자는 \`brand\_result.json\`의 \`status\`와 \`errors\`를 통해 전체 처리 결과를 확인할 수 있습니다.
...
API 오류 또는 사용량 제한이 발생할 경우 \`safe\_generate()\`가 해당 오류를 처리하고 다음 단계로 진행하며, 최종 오류 내용은 \`brand\_result.json\`의 \`errors\`에 기록됩니다.
